defmodule OurExpensesWeb.CoreComponents do
  @moduledoc """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(OurExpensesWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(OurExpensesWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  ## Examples
      ...
      <.threshold_badge value={@amount}/>
      <.threshold_badge value={2000.0} symbol="R$"/>
      <.threshold_badge value={budget} color={:blue}/>
  """
    ...
    >
      ...
      <div class="inline-flex gap-2 self-end rounded bg-green-100 p-1 text-green-600">
        ...
        <span class="text-xs font-medium">
          <%= :erlang.float_to_binary(@balance / @budget * 100, decimals: @precision) %>
        </span>
      </div>
      <br />
      <div>
        <strong class="block text-sm font-medium text-gray-500"> used </strong>
        <p>
          <span class="text-2xl font-medium text-gray-900">
            $<%= :erlang.float_to_binary(@balance, decimals: @precision) %>
          </span>
          <strong class="text-sm font-medium text-gray-500"> from </strong>
          <span class="text-2xl font-medium">
            $<%= :erlang.float_to_binary(@budget, decimals: @precision) %>
          </span>
        </p>
      </div>
    </article>
